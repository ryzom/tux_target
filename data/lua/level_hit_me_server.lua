...
function CEntity:setTeamScore( score )
  --print("setTeamScore");
  --print(self.base:currentScore());
  --print(score);
  if(score==0) then
    self.bonusTime = 0;
  end
  if(self.base:currentScore() and score==0) then
    self.bonusTime = 0;
    self.base:displayText(0,12,1,255,255,0,"bonus time left : 0",15);
  end
  
  if(self.base:currentScore()==0 and score~=0) then
    self.bonusTime = timeRemaining() * 5;
    local bonus_string = "bonus time left : " .. self.bonusTime;
    self.base:displayText(0,11,1,255,255,0,bonus_string,15);
  end
  
  --if(self.base:currentScore()==score) then
  --  return;
  --end
...
function Entity:init()
  ...
  self:displayText(0,12,1,255,255,255,"warning : Team share the score",10);
  if(t==0) then
  	self:displayText(0,13,1,255,0,0,"You are in RED team",15);
  	self:displayText(0,14,1,255,0,0,"Land on RED target !",20);
  else
  	self:displayText(0,13,1,100,100,255,"You are in BLUE team",15);
  	self:displayText(0,14,1,100,100,255,"Land on BLUE target !",20);
  end
...
function entityEntityCollideEvent ( entity1, entity2 )
  ...
  if(entity1:parent().team==0) then
  	entity1:displayText(0,12,1,100,100,255,"you have been hit",15);
  else
  	entity1:displayText(0,12,1,100,100,255,"you hit someone",15);
  end
end
...
function levelPostUpdate()
  ...
  if(currentTeamRedScore~=teamRedScore) then
    displayTextToAll(0,13,1,255,0,0,currentTeamRedScore,10);
    teamRedScore = currentTeamRedScore;
  end

  if(currentTeamBlueScore~=teamBlueScore) then
    displayTextToAll(0,14,1,100,100,255,currentTeamBlueScore,10);
